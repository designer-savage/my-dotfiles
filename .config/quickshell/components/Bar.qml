...
PanelWindow {
    ...
    Process {
        id: networkProc
        command: ["bash", "-c", "wifi=$(LANG=C nmcli -t -f active,ssid,signal dev wifi 2>/dev/null | grep '^yes' | head -1); if [ -n \"$wifi\" ]; then ssid=$(echo \"$wifi\" | cut -d: -f2); sig=$(echo \"$wifi\" | cut -d: -f3); echo \"1|$ssid|$sig\"; else echo '0||0'; fi; bt='0'; devices=$(echo -e 'devices\\nquit' | bluetoothctl 2>/dev/null | grep '^Device' | awk '{print $2}'); for mac in $devices; do if echo -e \"info $mac\\nquit\" | bluetoothctl 2>/dev/null | grep -q 'Connected: yes'; then bt='1'; break; fi; done; echo \"bt:$bt\""]
        stdout: SplitParser {
            onRead: data => {
                var line = data.trim()
    ...
    Timer {
        ...
        triggeredOnStart: true
        onTriggered: {
            if (!batteryPercentProc.running) batteryPercentProc.running = true
            if (!batteryStatusProc.running) batteryStatusProc.running = true
        }
    }

    Process {
        id: batteryPercentProc
        command: ["bash", "-c", "cat /sys/class/power_supply/BAT*/capacity 2>/dev/null | head -1 || echo 100"]
        stdout: SplitParser {
            onRead: data => {
                bar.batteryPercent = parseInt(data) || 100
                var cap = bar.batteryPercent
                if (cap >= 90) bar.batteryIcon = "󰁹"
                else if (cap >= 80) bar.batteryIcon = "󰂂"
                else if (cap >= 70) bar.batteryIcon = "󰂁"
                else if (cap >= 60) bar.batteryIcon = "󰂀"
                else if (cap >= 50) bar.batteryIcon = "󰁿"
                else if (cap >= 40) bar.batteryIcon = "󰁾"
                else if (cap >= 30) bar.batteryIcon = "󰁽"
                else if (cap >= 20) bar.batteryIcon = "󰁼"
                else if (cap >= 10) bar.batteryIcon = "󰁻"
                else bar.batteryIcon = "󰁺"
                if (bar.batteryPercent <= 10) bar.batteryClass = "critical"
    ...
    Process {
        ...
        command: ["bash", "-c", "status=$(cat /sys/class/power_supply/BAT*/status 2>/dev/null || echo Unknown); time=$(acpi -b 2>/dev/null | grep -oP '\\d+:\\d+' | head -1); echo \"$status|$time\""]
        stdout: SplitParser {
            onRead: data => {
                var parts = data.trim().split("|")
                var status = parts[0]
                ...
                if (status === "Charging") {
                    bar.batteryCharging = true
                    bar.batteryClass = "charging"
                    bar.batteryIcon = "󰂄"
                    bar.batteryTime = time
                } else if (status === "Full") {
                    bar.batteryCharging = false
                    bar.batteryTime = ""
                } else {
                    bar.batteryCharging = false
                    bar.batteryTime = time
    ...
    Process {
        id: ramProc
        command: ["bash", "-c", "free -h | grep Mem | awk '{used=$3; total=$2; gsub(/Gi/, \"\", used); gsub(/Gi/, \"\", total); pct=int(($3/$2)*100); print pct \"|\" used \"|\" total}'"]
        stdout: SplitParser {
            onRead: data => {
                var parts = data.trim().split("|")
                bar.ramPercent = parseInt(parts[0]) || 0
                bar.ramUsed = parts.length > 1 ? parts[1] : "0"
                bar.ramTotal = parts.length > 2 ? parts[2] : "0"
            }
        }
    }
    ...
    Process {
        id: kbProc
        command: ["bash", "-c", "hyprctl devices -j | jq -r '.keyboards[] | select(.main == true) | .active_keymap' | head -1 | cut -c1-2"]
        stdout: SplitParser {
            onRead: data => {
                bar.kbLayout = data.trim().toLowerCase()
            }
        }
    }
            ...
            Notch {
                ...
                Item {
                    ...
                    Row {
                        ...
                        Text {
                            id: clockDate
                            visible: clockNotch.expanded
                            color: root.walColor8
                            font.pixelSize: 11
                            font.bold: true
                            font.family: "JetBrainsMono Nerd Font"
                            opacity: clockNotch.expanded ? 1 : 0
                ...
                MouseArea {
                    ...
                    hoverEnabled: true
                    cursorShape: Qt.PointingHandCursor
                    onClicked: clockNotch.expanded = !clockNotch.expanded
                }
                ...
                Timer {
                    ...
                    onTriggered: {
                        ...
                        if (clockNotch.expanded) {
                            clockDate.text = Qt.formatDateTime(now, "dd.MM.yyyy")
                        }
                    }
                }
            }
        ...
        Row {
            ...
            Notch {
                    ...
                    Row {
                        ...
                        Row {
                            ...
                            Text {
                                ...
                                font.bold: true
                                font.family: "JetBrainsMono Nerd Font"
                                opacity: batteryNotch.expanded ? 1 : 0
                                Behavior on opacity { NumberAnimation { duration: 200; easing.type: Easing.OutCubic } }
                            }
                        }
                    }
                ...
                MouseArea {
                    ...
                    hoverEnabled: true
                    cursorShape: Qt.PointingHandCursor
                    onClicked: batteryNotch.expanded = !batteryNotch.expanded
                }
            }
            ...
            Notch {
                id: ramNotch
                property bool expanded: false
                width: ramLabel.implicitWidth + 24
                hovered: ramMA.containsMouse
                tooltip: "RAM: " + bar.ramUsed + "/" + bar.ramTotal + "GB"
                ...
                Item {
                    ...
                    Text {
                        id: ramLabel
                        anchors.centerIn: parent
                        text: ramNotch.expanded ? "󰍛 " + bar.ramUsed + "/" + bar.ramTotal + "GB" : "󰍛 " + bar.ramPercent + "%"
                        color: bar.ramPercent > 80 ? root.walColor1 : root.walColor5
                        font.pixelSize: 11
                        font.bold: true
                        font.family: "JetBrainsMono Nerd Font"
                ...
                MouseArea {
                    ...
                    hoverEnabled: true
                    cursorShape: Qt.PointingHandCursor
                    onClicked: ramNotch.expanded = !ramNotch.expanded
                }
            }

            Notch {
                width: kbLabel.implicitWidth + 24
                hovered: kbMA.containsMouse
                ...
                Item {
                    ...
                    Text {
                        id: kbLabel
                        anchors.centerIn: parent
                        text: bar.kbLayout.toUpperCase()
                        color: bar.kbLayout === "ru" ? root.walColor4 : root.walColor5
                        font.pixelSize: 11
                        font.bold: true
                        font.family: "JetBrainsMono Nerd Font"
                ...
                MouseArea {
                    ...
                    hoverEnabled: true
                    cursorShape: Qt.PointingHandCursor
                    onClicked: switchKbProc.running = true
                }
                Process {
                    id: switchKbProc
                    command: ["hyprctl", "switchxkblayout", "all", "next"]
                    onExited: { if (!kbProc.running) kbProc.running = true }
                }
            }
